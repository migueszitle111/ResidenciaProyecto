// app/Perfil/Perfil_Actualizar/page.js
"use client";
...
const Perfil_Actualizar = () => {
  ...
  const [idprofessional, setIdprofessional] = useState(session?.user?.idprofessional || "");
  const [specialty, setSpecialty]         = useState(session?.user?.specialty || "");
                ...
                <form onSubmit={handleUpdateProfile}>
                  ...
                  {status === "authenticated" && session && (
                    <figure className="flex items-start sm:items-center">
                      <figcaption>
                        <h5 className="font-semibold text-lg">Modificar Perfil</h5>
                        <hr />
                        <div className="mb-4">
                        ...
                        <div className="mb-4">
                          ...
                          <input
                            className="appearance-none border border-gray-200 bg-gray-100 text-black rounded-md py-2 px-3 hover:border-gray-400 focus:outline-none focus:border-gray-400 w-full"
                            type="text"
                            placeholder={session.user?.specialty}
                            required
                            value={specialty}
                        ...
                        <div className="mb-4">
                          <label className="block mb-1">Cédula profesional:</label>
                          <input
                            className="appearance-none border border-gray-200 bg-gray-100 text-black rounded-md py-2 px-3 hover:border-gray-400 focus:outline-none focus:border-gray-400 w-full"
                            type="text"
                            placeholder={session.user?.idprofessional}
                            required
