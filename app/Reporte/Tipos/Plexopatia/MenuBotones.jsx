...
function LinkUploaderModal({ pdfBlob, pdfFilename, nombrePaciente, session, onClose }) {
  ...
  const [title, setTitle]       = useState(`Plexopatía – ${nombrePaciente || [session?.user?.name, session?.user?.lastname].filter(Boolean).join(' ')}`.trim());
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const finalTitle = (title.trim() || `Plexopatía – ${nombrePaciente || 'Paciente'}`).slice(0, 140);
...
// Renders the image clipped to a circle on a canvas, returns PNG data URL with transparent corners
...
export default function ExportBar({ nombrePaciente, textoReporte, activeOv = [], figuras = [], laminaSize = { w: 690, h: 620 }, listaVisual = [], imgLista = null, comentarioLista = '', onBack: _onBack, onReset, isOpen = false, onClose: notifyClose }) {
  ...
  const buildPayload = async (plantillaId) => {
    ...
    const figurasB64 = await Promise.all(
      figuras.map(async (f) => {
        ...
        return { ...f, src, x: Math.round(f.x * scaleX), y: Math.round(f.y * scaleY), nw: f.nw, nh: f.nh };
      })
    );
    return {
  ...
  const handlePlantillaSelect = async (id) => {
    ...
    try {
      ...
      setProgress(40);
      const res = await fetch('/api/pdf/generate-pdf/plexopatia', {
        method: 'POST',
