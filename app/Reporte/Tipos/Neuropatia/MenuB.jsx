...
function LinkUploaderModal({ pdfBlob, pdfFilename, nombrePaciente, session, onClose }) {
  ...
  const [title, setTitle]           = useState(`Neuropatía – ${nombrePaciente || [session?.user?.name, session?.user?.lastname].filter(Boolean).join(' ')}`.trim());
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const finalTitle = (title.trim() || `Neuropatía – ${nombrePaciente || 'Paciente'}`).slice(0, 140);
      const folder = toSafeToken(nombrePaciente || 'paciente');

      const initRes = await fetch('/api/share/init', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: finalTitle, message: message.trim() || undefined,
          expiresInSeconds: expirySeconds, studyType: STUDY_KEY,
          patient: nombrePaciente || null, doctor: doctor || null,
