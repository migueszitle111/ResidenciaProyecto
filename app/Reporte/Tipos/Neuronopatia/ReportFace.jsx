...
{/* ───── Overlay flotante guía de menú (arrastrable) ───── */}
{showOverlay && (
  <div
    style={{
      position: 'absolute',
      inset: 0,                     // ocupa todo el vertical-container
      pointerEvents: 'none',        // la IU sigue clic-able
      zIndex: 99999
    }}
    onClick={handleCloseOverlay}
  >
    <Rnd
      position={overlayPos}                   // posición controlada
      size={{ width: 360, height: 100 }}      // sin redimensionar
      bounds="parent"
      enableResizing={false}
      onDragStart={() => setIsDraggingOverlay(true)}
      onDragStop={(e, d) => {
        setIsDraggingOverlay(false);
        setOverlayPos({ x: d.x, y: d.y });    // guarda nueva posición
        /*  //  Si quieres persistirla entre sesiones:
            localStorage.setItem('menuGifPos', JSON.stringify({ x: d.x, y: d.y }));
        */
      }}
      style={{
        cursor: isDraggingOverlay ? 'grabbing' : 'grab', // 🖐️  mano / mano cerrada
        pointerEvents: 'auto'             // el bloque SÍ recibe eventos
      }}
    >
      <img
        src="/assets/Gifs/Ayudaboton.gif"
        alt="Guía rápida"
          draggable={false}          //  👈 evita el 🚫

        style={{ width: '100%', height: '100%' }}
        onClick={e => e.stopPropagation()}      // evita cerrar al hacer clic
      />
