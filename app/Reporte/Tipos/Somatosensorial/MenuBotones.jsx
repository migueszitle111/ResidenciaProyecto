...
// ─── Constantes ───────────────────────────────────────────────────────────────
const STUDY_KEY    = 'Somatosensorial';
const STUDY_PREFIX = 'mEDXproSomatosensorial';
...
// Contador global para pacientes sin nombre → mEDXproSomatosensorial_1, _2, …
...
export default function ExportBar({ nombrePaciente, textoReporte, activeOv = [], figuras = [], laminaSize = { w: 690, h: 620 }, listaVisual = [], imgLista = null, comentarioLista = '', onBack: _onBack, onReset, isOpen = false, onClose: notifyClose }) {
  ...
  const handlePlantillaSelect = async (id) => {
    ...
    try {
      ...
      setProgress(40);
      const res = await fetch('/api/pdf/generate-pdf/somatosensorial', {
        method: 'POST',
    ...
    <>
      ...
      {showLinkUploader && (
        <LinkUploaderModal
          ...
          studyKey={STUDY_KEY}
          defaultTitle={nombrePaciente ? `Vías Somatosensoriales – ${nombrePaciente}` : 'Vías Somatosensoriales'}
          fallbackTitle={`Vías Somatosensoriales – ${nombrePaciente || 'Paciente'}`}
        />
      )}
    </>
  );
