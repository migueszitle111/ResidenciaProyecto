...
export async function getAuthenticatedRequestUser(request) {
  ...
  console.log("[AUTH] authHeader present:", !!authHeader, "| starts Bearer:", authHeader.startsWith("Bearer "));
  if (authHeader.startsWith("Bearer ")) {
    const mobileToken = authHeader.slice(7).trim();
    const mobileSecret = process.env.MOBILE_JWT_SECRET;
    console.log("[AUTH] mobileSecret present:", !!mobileSecret, "| tokenLen:", mobileToken.length);
