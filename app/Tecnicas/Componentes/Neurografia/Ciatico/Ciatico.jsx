...
const Ciatico = () => {
    ...
    return (
        ...
        <div>
            {/* Primera Imagen */}
            {currentImageIndex === 0 && <button className="btnCt1" onClick={() => handleButtonClick('SACRO. Se inserta la aguja de estimulación monopolar alrededor de 1 cm medial y ligeramente caudal a la espina ilíaca postero superior, el ánodo será un electrodo de superficie colocado sobre la apófisis espinosa. Con este registro se puede determinar la conducción a través del plexo sacro, posterior a la evaluación del nervio Ciático y restando el cálculo de su latencia.', { top: '62%', left: '50%' }, 'E')}></button>}
            {currentImageIndex === 0 && <button className="btnCt2" onClick={() => handleButtonClick('GLÚTEO. Se inserta una aguja monopolar (cátodo) de 0.75 mm en el pliegue glúteo a media distancia de la intersección entre la tuberosidad isquiática y el trocánter mayor, u horizontalmente con referencia a una línea trazada desde el vértice del hueco poplíteo. Electrodo de superficie cercano a aguja de estimulación como ánodo.', { top: '62%', left: '50%' }, 'E')}></button>}
            {currentImageIndex === 0 && <button className="btnCt3" onClick={() => handleButtonClick('Entre registro y estimulación.', { top: '62%', left: '50%' }, 'T')}></button>}
