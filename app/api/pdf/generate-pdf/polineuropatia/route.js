...
function resolveOverlayPaths(activeOv) {
  const paths = [];
  for (const key of activeOv) {
    const group = OVERLAY_GROUPS[key];
    if (group) {
      for (const gk of group) { const p = OVERLAYS_POLI[gk]; if (p && !paths.includes(p)) paths.push(p); }
    } else {
      const p = OVERLAYS_POLI[key];
...
export async function POST(req) {
  try {
    ...
    const ovPaths = resolveOverlayPaths(activeOv);

    const [baseImgBytes, imgListaBytes, ...overlayBytesArr] = await Promise.all([
      fetchLocalBytes('/PolineuropatiaImg/BP_TR.png'),
    ...
    return new NextResponse(pdfBytes, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename=reporte_polineuropatia.pdf',
      },
    });
  } catch (err) {
    console.error('Error generando PDF polineuropatia:', err);
