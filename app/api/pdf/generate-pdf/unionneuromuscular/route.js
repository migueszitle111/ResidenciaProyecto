...
export async function POST(req) {
  ...
  try {
    ...
    const ovPaths = activeOv.map(k => OVERLAYS_UN[k]).filter(Boolean);
    const [baseImgB64, imgListaB64, doctorLogoB64, ...overlayB64s] = await Promise.all([
      localImgToB64('/UnionNeuromuscularImg/BP_UTR.png'),
    ...
    console.error('Error PDF union neuromuscular:', err);
