...
// ── Mapa de overlays (debe coincidir con OVERLAYS_AUDITIVA en ReportFace.jsx) ──
...
function buildPage1Html({ finalConclusion, userData, baseImgB64, overlayB64s, figuras, topLeftText }) {
  ...
  // Handle two-paragraph conclusion (separated by \n\n)
  const conclusionHtml = finalConclusion
    .split('\n\n')
    .map(p => `<p style="margin-bottom:8px">${esc(p.trim())}</p>`)
    .join('');
...
<div class="page">
  ...
  <div class="diag">
    ...
    <div class="diag-text">${conclusionHtml}</div>
  </div>
...
export async function POST(req) {
  ...
  try {
    ...
    const ovPaths = activeOv.map(k => OVERLAYS_AUDITIVA[k]).filter(Boolean);

    const [baseImgB64, imgListaB64, doctorLogoB64, ...overlayB64s] = await Promise.all([
      localImgToB64('/AuditivaImg/AU_BASE_TR.png'),
    ...
    console.error('Error PDF auditiva:', err);
