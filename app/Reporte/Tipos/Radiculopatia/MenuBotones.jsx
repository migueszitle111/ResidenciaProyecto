...
export default function ExportBar({ nombrePaciente, textoReporte, flowType = '', activeOv = [], postOv = [], antOv = [], crosses = [], figuras = [], laminaSize = { w: 690, h: 620 }, listaVisual = [], imgLista = null, comentarioLista = '', onBack: _onBack, onReset, isOpen = false, onClose: notifyClose }) {
  ...
  const handlePlantillaSelect = async (id) => {
    ...
    try {
      ...
      setProgress(40);
      const res = await fetch('/api/pdf/generate-pdf/radiculopatia', {
        method: 'POST',
    ...
    <>
      ...
      {showLinkUploader && (
        <LinkUploaderModal
          ...
          studyKey={STUDY_KEY}
          defaultTitle={nombrePaciente ? `Radiculopatía – ${nombrePaciente}` : 'Radiculopatía'}
          fallbackTitle={`Radiculopatía – ${nombrePaciente || 'Paciente'}`}
          initialJustReport={true}
          requireUserFileWhenNotJustReport={false}
          useSlugFallback={true}
        />
      )}
    </>
  );
