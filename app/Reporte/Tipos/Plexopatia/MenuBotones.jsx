...
function LinkUploaderModal({ pdfBlob, pdfFilename, nombrePaciente, session, onClose }) {
  ...
  const [title, setTitle]       = useState(nombrePaciente ? `Plexopatía – ${nombrePaciente}` : 'Plexopatía');
  ...
  const buildPayload = async (plantillaId) => {
    const figurasB64 = await Promise.all(
      figuras.map(async (f) => {
        ...
        // Send raw screen-pixel coords and sizes; server applies one scale using laminaSize.
        return { ...f, src,
          nw: f.nw ?? defSz,
          nh: f.nh ?? defSz,
          dw: f.dw ?? defSz,
